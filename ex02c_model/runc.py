...
./ex02/runc.py ex02c_model/ conll15-st-03-04-15-dev/ conll15-st-03-04-15-dev/ conll15-st-03-04-15-dev/ ex02c_model/

[2015-05-13 21:42] learning epoch 359 (26.58%), ex02c_model/
[2015-05-13 21:43] learning epoch 359 (51.90%), ex02c_model/
[2015-05-13 21:45] learning epoch 359 (77.22%), ex02c_model/
[2015-05-13 21:46] learning epoch 359 (316.75 sec), rate 9.85e-05, train cost (0.5789 0.6717) avg 0.61440122203 +
[2015-05-13 21:46]   -0.146763409335 0.527320019806 0.213791948896
[2015-05-13 21:47] valid set (89.23 sec): precision 0.00, recall 0.00, f1 0.00 +
[2015-05-14 15:17] learning epoch 518 (26.58%), ex02c_model/
[2015-05-14 15:19] learning epoch 518 (51.90%), ex02c_model/
[2015-05-14 15:20] learning epoch 518 (77.22%), ex02c_model/
[2015-05-14 15:21] learning epoch 518 (329.87 sec), rate 9.70e-06, train cost (0.5757 0.6502) avg 0.602450291567 +
[2015-05-14 15:21]   -0.145223955282 0.521814531163 0.201055157034
[2015-05-14 15:22] valid set (89.32 sec): precision 0.00, recall 0.01, f1 0.01
"""
...
class RNN_deep3_bi(object):
    ...
    def __init__(self, x_dim, hidden_dim, y_dim, w_spread, p_drop):
        ...
        def model(x, wfx, hfx_0, wbx, hbx_0, wf1, hf1_0, wb1, hb1_0, wy, p_drop):
            ...
            one = np.float32(1.)
            if p_drop > 0.:
                masks = dropout_masks(theano.shared(np.float32(p_drop)), [hfx_0.shape, hbx_0.shape, hf1_0.shape, hb1_0.shape])
            else:
                masks = [[]] * 4
        ...
        l2_reg = 0.001
        l2 = sum([ T.mean(w ** 2)  for w in [self.wfx, self.wbx, self.wf1, self.wb1, self.wy] ])
...
if __name__ == '__main__':
    ...
    learn_rate = 0.001  # dataset trial=0.01, dev=0.001, train=0.0001?
    decay_after = 10
    decay_rate = 0.9
    ...
    while epoch < epochs or learn_rate > decay_min:
        ...
        if epoch % valid_freq == 0:
            ...
            log.info("valid set ({:.2f} sec): precision {:.2f}, recall {:.2f}, f1 {:.2f}{}".format(time.time() - t, precision, recall, f1, (" +" if f1 > best_f1 else "")))
